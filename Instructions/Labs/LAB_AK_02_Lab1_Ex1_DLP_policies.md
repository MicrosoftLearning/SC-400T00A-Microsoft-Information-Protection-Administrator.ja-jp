---
ms.openlocfilehash: 32335d8269e064df0cff0b7b4c1754b4e4cd844b
ms.sourcegitcommit: c78ddb966a982b76e7f46fc17a42847f76d6e1a0
ms.translationtype: HT
ms.contentlocale: ja-JP
ms.lasthandoff: 07/15/2022
ms.locfileid: "147154845"
---
...
1. **[DLP ポリシーの名前の設定]** ページで、 **[名前]** フィールドに *「クレジット カードの DLP ポリシー」* と入力し、 *「クレジット カード番号の共有を防止する」* と **[説明]** フィールドに入力します。 **[次へ]** を選択します。
...
1. **[ルールの作成]** ページの **[ユーザーによる上書き]** セクションで、 **[Allow overrides from M365 services]\(M365 サービスからの上書きを許可する\)** の下にある **[Allow overrides from M365 services.Exchange、Sharepoint、OneDrive、Teams のユーザーがポリシー制限を上書きするのを許可します]** チェック ボックスをオンにします。

    注: **[M365 サービスからの上書きを許可します]** チェック ボックスを選択できなかった場合、 **[ポリシー ヒントを使用して Office 365 のユーザーに通知する]** チェック ボックスをオンにします。これは、前の手順の **[ルールの作成]** ページにある **[ユーザー通知] >> [Microsoft 365 サービス]** セクションの下にあります。 次に、 **[M365 サービスからの上書きを許可します。Exchange、Sharepoint、OneDrive、Teams のユーザーがポリシー制限を上書きするのを許可します]** チェック ボックスをオンにします。
...
1. **Microsoft Edge** で、Microsoft Purview ポータルのタブがまだ開かれているはずです。 右上にある Joni Sherman の **プロファイル画像** を選択し、 **[サインアウト]** を選択します。その後、ブラウザーを閉じます。
...
1.  **Microsoft Edge** に、Microsoft Defender for Cloud Apps ポータルのタブがまだ開かれているはずです。 右上にあるある MOD 管理者の **プロファイル画像** を選択し、歯車の横にある **[サインアウト]** を選択します。 その後、ブラウザーを閉じます。
